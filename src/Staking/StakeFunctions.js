...
const STAKE = require("../abi/EthStake.json");
...
export async function stakeEther(numberOfDays, contractAddress, amount, account, signer) {
  // Create a contract instance
  const stakingContract = new ethers.Contract(ethStakeAddress, STAKE, signer);

  // Convert the amount to the appropriate format
  const amountToStake = ethers.utils.parseEther(amount.toString());

  // Call the stakeEther function on the contract
  await stakingContract.stakeEther(numberOfDays, {
    value: amountToStake,
  });

  // Optionally, you may want to wait for the transaction to be mined
  // await stakingContract.provider.waitForTransaction(tx.hash);

  // Log the stake information
  console.log(`Successfully staked ${amount} Ether for ${numberOfDays} days`);
}
