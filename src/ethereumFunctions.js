...
const FARM = require("./abi/Farming.json");
const ETHSTAKE = require("./abi/EthStake.json");
const USDTSTAKE = require("./abi/UsdtStake.json");


export async function stakeEther(amount, contractAddress, signer) {
  try {
    const contract = new ethers.Contract(contractAddress, ETHSTAKE.abi, signer);
    const valueInWei = ethers.utils.parseEther(amount);

    // Call the stakeEther function in the contract
    const transaction = await contract.stakeEther({
      value: valueInWei,
    });
    ...
    console.log(`Successfully staked ${amount} ETH.`);
  } catch (error) {
    console.error('Error in stakeEther function:', error);
  }
}

export async function stakeLP(numberOfDays, lpAmount, contractAddress, signer) {
  try {
    const contract = new ethers.Contract(contractAddress, FARM, signer);
    const transaction = await contract.stakeLP(numberOfDays, lpAmount);

    // Wait for the transaction to be mined
    await transaction.wait();

    console.log(`Successfully staked ${lpAmount} LP tokens for ${numberOfDays} days.`);
  } catch (error) {
    console.error('Error in stakeLP function:', error);
  }
}

export async function stakeUSDT(numberOfDays, lpAmount, contractAddress, signer) {
  try {
    const contract = new ethers.Contract(contractAddress, USDTSTAKE, signer);
    const transaction = await contract.stakeLP(numberOfDays, lpAmount);
